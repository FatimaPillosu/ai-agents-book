...
> **Consequence to plan for.** The 51 SVG files under `figures/` were rendered from v1.0 briefs and are now out of date. They remain correct as to structure, role-colour and layout, but they carry the sparse v1.0 label set. Re-rendering the set against the v2.0 descriptions is a separate job, not done by the brief rewrite. **[AUTHOR: decide whether the set is re-rendered in one batch before release, or chapter by chapter as each is finalised.]**
