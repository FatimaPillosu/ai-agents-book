...
- **Style guide** (`STYLE.md`, v2.0 — personal/conversational voice, info-boxes, sentence-per-line) and **figure guide** (`FIGURES.md`, v1.0) are in place and binding. **STYLE.md v2.0 applies retroactively to all chapters**; Ch. 1 is already in v2.0 voice, the rest are revoiced in revision pass R1.
- All 17 chapters (`ch01`–`ch17`) are drafted first-pass in `/manuscript` and carry `[AUTHOR: …]` markers for lived/executed material. **Revision pass R1 is under way**, governed by `manuscript/REVISION-PLAN.md` (executed by ai-writer, reviewed by ai-reviewer).
...
**Decided:** single ~150 pp volume · audience: environmental/geosciences · self-published, free, LinkedIn-first · unified repository with newsletter as update channel · no formal index · specification (Ch. 3) and manuscripts (Ch. 9) chapters in · exercises live in `/exercises`, not the page budget · cross-domain examples deferred to a second edition · manuscript in plain Markdown · budgets indicative · vendor-neutral naming in print · figures governed by `FIGURES.md` · **STYLE.md v2.0 voice applies retroactively to all chapters** (24 Jul 2026) · **de-duplication now**: each recurring idea has one canonical home and short cross-references elsewhere (24 Jul 2026) · **references policy**: key claims cited from verified sources in the `/research` reports (ai-researcher, DOI/URL per source); annotated further reading in the back matter; no citation ever from agent memory (24 Jul 2026) · full chapter anatomy binds Part II only, with Ch. 11 and Ch. 12 still owing verification checklists and repository pointers (24 Jul 2026).
...
## Writing style (condensed from STYLE.md v2.0 — binding)
