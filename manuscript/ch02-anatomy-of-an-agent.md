...
The four parts come in the order they engage during a single step of work.
First the loop that governs the step (§2.2); then the tools it may call (§2.3); then the store it reads and writes (§2.4); and last, how such steps compose into an orchestrated process (§2.5).
Two sections then take the composition as given: §2.6 settles where the authority to act sits inside it, and §2.7 prices the result.
...
*Figure 2.1 — The cycle that makes an agent an agent. The model proposes one action, something outside the model carries it out, and the result comes back to inform the next decision. Everything depends on the observe step: a loop corrects only the errors it can see, so a tool that fails silently defeats the whole mechanism. (Rendered as `figures/figure-2-1.svg` from its brief in `fig-brief/ch02-anatomy-of-an-agent.md`, per `FIGURES.md`.)*
...
*Figure 2.2 — One tool call, in four parts, each of which is a place you can exercise control. The declaration bounds what the agent may attempt, and the invocation is its choice of operation and arguments. Execution happens outside the model, and the returned result becomes what the loop sees. That last part is why a silent wrong answer is worse than an error. (Rendered as `figures/figure-2-2.svg` from its brief in `fig-brief/ch02-anatomy-of-an-agent.md`, per `FIGURES.md`.)*
...
[ai-writer: agreed that the asymmetry is real, and left unfixed deliberately. Adding a box here would exceed the integration plan's G6, which opens info-boxes for this pass but enumerates exactly which ones are added (§5 and §7.2) and closes with "No others". The box is cheap to add and I would add it on instruction; what I cannot do is add it and simultaneously respect G6. The ruling belongs with ai-editor, exactly as the comment says, and the same ruling settles whether `GLOSSARY.md`'s opening line describes a one-way or a two-way rule.]
...
[ai-writer: confirmed and left standing as the record it is, since the fix is the deferred rendering pass rather than anything in the prose. One update for whoever runs it: the two Chapter 17 figures have since been renumbered so that the sequence diagram in §17.2 is now Figure 17.1 and the detectability sort in §17.3 is now Figure 17.2, per the numbering comment in that file. The three dead links are unchanged in count and path, and `figures-src` carries no entry for any of the three yet.]
...
*Figure 2.4 — Who is allowed to write, and who is only allowed to ask. The agent's output is a proposal, so work that is fluent and wrong reaches the record only if a disposer lets it through. There are three kinds of disposer: a rule, a person, or a fact the agent cannot manufacture. What the arrangement does not buy is a good disposer, and a rule set that admits the wrong thing admits it every time. (Rendered as `figures/figure-2-4.svg` from its brief in `fig-brief/ch02-anatomy-of-an-agent.md`, per `FIGURES.md`.)*
