...
Chapter 1 left us with a taxonomy — a model inside an agent inside a workflow — and in this chapter I want to open the agent up and show you what each of its parts does and, just as importantly, where each one fails.
My purpose here is diagnostic rather than architectural.
If you understand an agent only as a black box that answers prompts, you have no way to reason about why it produced a wrong answer, and nowhere to attach the verification that Part III will insist on.
Taken apart, an agent turns out to have four parts and no more: a model that proposes, a loop that sequences, tools that act, and a store that remembers.
...
So the account that follows describes each part as an instrument-part: what it is for, what it can be relied on to do, and the specific way it misleads you.
...
This plan–act–observe loop is the agent's engine, and it is worth stating its steps precisely, because loose description is where misplaced expectation begins.
...
In the observe step the result of that execution — a returned value, an error message, a retrieved passage — is added to the state and passed back to the model, which plans again.
...
> **In plain terms — Plan–act–observe loop.** The cycle at the heart of every agent: the model proposes one action, something outside the model carries it out, the result comes back, and the model uses it to decide what to do next. Round and round until the work is done or a stop condition halts it. It is what turns a one-shot answer into a process that can, in the right conditions, correct itself.
...
A loop only corrects the errors it can observe, so the quality of the observe step — whether the tool returns a real error or a silent wrong answer, whether the check is external or the model is left to grade its own homework — decides whether iteration converges on a correct result or merely piles up confident wrongness.
The loop's characteristic failure follows directly from this.
Give it a weak or absent observe step and the very mechanism that recovers from a traceback will instead elaborate a mistake across many steps, each one locally plausible, arriving at an output whose length and detail read as thoroughness while its foundation is an early error nobody ever checked — the plausible failure of Chapter 1, now compounded step by step (high confidence in the mechanism; how often it bites varies by task and model).
...
Tools are how an agent hands its own weaknesses to machinery that does not share them, and I find that framing — delegation of weakness, rather than extension of strength — is the one that predicts which tools actually matter.
...
But each of them already has a mature, deterministic implementation, and a tool is simply a declared interface through which the model can call that implementation and get its result back.
The point was made early and cleanly in the research literature: a model can be taught when to reach for a calculator, a search index or a lookup, precisely because it is bad at the arithmetic and factual recall that trivial specialised systems do perfectly (Schick et al., 2023).
The canonical cases are just the model's canonical weaknesses turned inside out.
...
Context and memory are the agent's working store, and the property that governs how you use them is that the store is finite, ordered and lossy — in ways you have to design around rather than wish away.
...
The window has a fixed capacity that, although it has grown by more than an order of magnitude across the capability period I described in Chapter 1 [verify], remains finite, and every tool result the loop appends eats into it.
It helps to borrow the vocabulary the research literature uses here, which separates the volatile working memory of a single run from the longer-term stores an agent writes to and reads back — episodic records of what happened, semantic notes of what was learned (Sumers et al., 2023) — because the distinction between the two is exactly where a scientific workflow either keeps its provenance or loses it.

Three consequences follow, and each one has caught people out.
The first is that a long-running loop fills its own context: every observed result takes up space, so a task of many steps will drift towards the window's limit, at which point earlier material — the original specification, an early constraint — has to be summarised, dropped or externalised, and a constraint that has fallen out of the window is a constraint the agent no longer honours.
The second is that position within the window matters: material at the start and the end of a long context is attended to more reliably than material buried in the middle, so merely fitting information into the window does not guarantee it is used (moderate confidence; the effect is documented but its magnitude varies by model).
The third is that the window is volatile — it holds only within a single run — so anything that must persist across runs needs an explicit, external memory: notes written to a file, records in a database, a specification kept under version control.
...
The design implication for scientific work is to treat context as a scarce, curated resource rather than a bucket you pour everything into.
...
Orchestration is the composition of individual agent steps into a larger process, and the question it answers is not how one agent thinks but how many steps, agents and gates you arrange so that the whole is more reliable than any part.
...
> **In plain terms — Orchestration.** Arranging several agent steps — and the checks and human decisions between them — into one larger workflow. Where a single agent is one worker, orchestration is the division of labour: who does what, in what order, and who checks whom. It is the layer where a workflow's reliability is actually engineered.

The simplest form is a sequence: a chain in which one step's output is the next step's input, which suits a task that decomposes into stages with a fixed order — acquire, then clean, then summarise.
...
> **In plain terms — Sub-agent.** A second agent that a coordinating agent hands a self-contained piece of work to, with its own tools and its own clean context. It does the messy or bulky part in isolation and passes back only the tidy result, so the coordinator never has to hold the mess in view.

A third form is a distinct reviewer step whose whole job is to check another step's output before it goes any further, and this one matters disproportionately in scientific work, because it is where the external verification of §2.2 is given a structural home.
...
These forms are not my invention; they are the composable patterns practitioners in the field converged on and documented — chaining, routing, parallel work, a coordinator with workers, and evaluator-and-improver loops — under a governing recommendation I share, which is to reach for the simplest arrangement that works and add orchestration only when the task visibly needs it (Anthropic, 2024).
...
I deliberately stop at the anatomy of composition here and leave the multi-agent patterns themselves for later.
...
The costs an agent runs up fall into three kinds, and they behave quite differently.
The first is token cost.

> **In plain terms — Token.** The unit these models read and write in — roughly a short chunk of a word. Text is billed and measured by the token, so the length of everything an agent reads and produces, including its own steadily growing transcript, is literally what you pay for.
...
The observation that reorganises all three, and that I return to throughout the book, is that in a well-run scientific workflow the dominant cost is not generation but verification.
...
The saving was never that the science becomes cheap to verify; it is that the production which precedes verification becomes cheap enough that you can afford to spend more of the budget verifying well — which is exactly where a scientific workflow should want to spend it.
...
*I have now taken the agent apart into a loop, tools, a finite store and their composition, and priced the result.*
*The next chapter turns to the human side of the same mechanism: how to specify a task so that this apparatus can execute it and a scientist can audit it afterwards — the skill that, in my experience, most failures trace back to.*
