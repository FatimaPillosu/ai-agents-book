...
[ai-writer: the substantive note is done. ch12 §12.4 now distinguishes the three by what each establishes rather than ranking them, and separates how demanding a property is to satisfy from how much evidential weight it carries. Three entries here follow it in one edit: "Reproducibility, replicability and auditability", "Auditability" and the closing clause of "Audit trail". Points (1) and (2) are left for ai-editor, as the comment asks. On (1) I have added no em-dash connectors to any entry I touched, so the new material stays on the stricter reading and a sweep remains possible either way. On (2) the propose–dispose entry still has no info-box in ch02 §2.6, and I have explained at that section why adding one would breach the integration plan's G6.] -->
...
**Reproducibility, replicability and auditability** (Chapter 12). Three different properties, routinely run together and worth keeping apart by what each one establishes. Reproducibility establishes that the procedure is deterministic: the same workflow on the same inputs returns the same answer. Replicability establishes that a finding is not an artefact of the route taken: an independent group asks the same question its own way and gets a compatible answer. Auditability establishes what was actually done, and has its own entry above. They are not one ranking: reproducibility is the most demanding to satisfy and says least once satisfied, and replicability carries the most weight, which is why Chapter 11 §11.2 puts independent-method corroboration at Tier 5 and re-running near Tier 1. An agentic workflow fails the first and can still reach the strongest by corroboration, because the same specification can return different work on a second run and the model behind a result can be withdrawn (Chapter 12 §12.4).
