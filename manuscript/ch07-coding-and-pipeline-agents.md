...
Exploratory analysis rewards exactly that mode of working: a forecast field is loaded, a regridding step is tried, the result is plotted, a threshold is nudged, and the investigation moves on, accumulating cells in the order the investigation happened to take rather than the order a reader would need.
That mode warrants no disparagement, because it is how understanding is actually built.
The trouble starts when the same artefact is pressed into service as though it were a pipeline, rerun next season, handed to a student, cited as the method behind a figure, or scheduled to produce an operational product, without acquiring any of the properties that would make it trustworthy in that role.
...
The reframing this chapter argues for is that the unit of work is not the code an agent produces but the set of gates that code must pass before it is trusted.
...
This chapter does not claim that gates make agent-written code safe in any absolute sense, because no arrangement of automated checks catches a plausible error that every check was too weak to notice.
The claim is narrower and more useful: the discipline agents make affordable is the same discipline good software engineering has always prescribed, and the right response to a fast, fallible code generator is to build the verification the field already knew it should have.
...
It is rather that the practice is front-loaded effort against a back-loaded and uncertain payoff, and human attention discounts the future steeply.
...
An author is the worst possible reviewer of their own code, because the same misunderstanding that produced an error also blinds the author to it.
...
What it can do is occupy the role of the tireless first reader that never skips the review because it is late on a Friday, applying a consistent checklist to every change and surfacing the ten routine issues so that the human's scarce attention lands on the one that matters.
...
The third gate is an independent reviewer agent, in which the word independent is load-bearing: it reads the proposed change against a review specification and reports what it finds, but it is deliberately a *different* agent instance from the one that wrote the code, with its own context, its own instructions and no stake in defending the work, for reasons §7.5 makes concrete.
...
The full treatment of that authority model, meaning least-privilege tool access and why it matters, is Chapter 12's; what belongs here is the shape it imposes on this pipeline.
...
The example that runs through this section takes a real exploratory analysis from the author's operational work and follows it through the redesign, so the gates are concrete rather than abstract.
...
The task handed to the workflow is not "write this analysis" but "turn this notebook into a governed pipeline", and that distinction shapes everything that follows.
...
These tests are written against the specification, and where the agent proposes them they are read before acceptance, because a test the agent both writes and satisfies asserts only that the code does what the code does.
...
Consequently, a transposed latitude and longitude, a metre-per-second field read as millimetres, or a fill value silently averaged into a mean will pass a green test suite whose assertions were derived from the very output they should have caught.
...
Even execution-based gates are not exempt from it.
...
A green suite confirms only that the code does what the suite asserts; it establishes nothing about whether the suite asserts the right thing (high confidence; this is the dominant failure mode of agent-written code).
...
A dependency updates and alters a default; an input format shifts a column; a regridding library changes how it handles edges.
In none of these cases is an error raised, because nothing is broken in a way the code checks for.
...
The anatomy of the behaviour, namely why a model asked to assess work tilts towards ratifying it, is the failure gallery's to dissect (Chapter 13), and the countermeasure at roster scale is Chapter 10's; here only its consequence for a coding pipeline and the configuration that answers it are needed.
The consequence is simple: asking an agent "is this correct?" straight after it has produced the thing invites the answer it has already given.
...
The exercises for this chapter, in `/exercises`, ask the reader to take one of their own run-once notebooks through the four gates and record which gate caught what, the same trace the worked example asks of the author.
