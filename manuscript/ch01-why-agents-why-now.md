...
> **Status:** draft r2 · voice v2.1 (`STYLE.md`) · sentence-per-line per `STYLE.md` §10 · figures as briefs per `FIGURES.md`.
...
The problem this book addresses is a growing mismatch between how much environmental scientists must attend to and how little time they have to attend to it.
The volume of data has grown relentlessly: satellite programmes now deliver observations at rates measured in terabytes a day, the big model-intercomparison archives run to tens of petabytes, and an operational forecasting centre produces more in a morning than any one person could read in a week [AUTHOR: verify one or two current figures — e.g. Copernicus daily volumes, CMIP6 archive size — and cite].
Every one of those streams carries obligations: it must be quality-controlled, reconciled with its neighbours, reprocessed when a version changes, and reported to funders and partners in formats that are themselves revised yearly.
What has not grown is the number of hours in anyone's week.
...
Software that can actually act, reading a file, running some code, looking at the result, and deciding what to do next, deserves a serious look.
The claims made for these systems are large, the commentary around them is sharply divided, and neither of those features is unusual for a technology at this stage of its development.
My own position falls between that of the enthusiasts and that of the sceptics: these systems are instruments, powerful and fallible, and fit for serious work only once they are instructed properly, checked, and kept under governance that matches the decisions resting on them.
The case for engaging now, rather than waiting for the technology to settle, rests on a small number of specific, datable changes in what these systems can do and in who can afford to use them, and that is where I begin.
...
Everything up to that point shared one limit that matters more in science than almost anywhere else: these systems produced text, and text on its own cannot run a quality-control pass, regrid a forecast field, or execute a test suite.
...
> **In plain terms — In-context learning.** The knack, which appeared as models grew larger, of picking up a new task from nothing more than an instruction and a couple of examples written into the conversation, with no retraining and no reprogramming. It is why you can steer these systems in ordinary written language.
...
It matters for science because it lets a model hand its own weak points to tools that do not share them: arithmetic goes to the interpreter, retrieval goes to the database, and the model is left doing the planning and interpretation in between.
...
You can even watch the trend in public numbers: on a widely used software-engineering benchmark, the share of real-world coding issues an agent could resolve unaided rose from around 2% at the benchmark's introduction in 2023 (Jimenez et al., 2023) to near-saturation about two years later, a climb from single-digit scores that one frontier-model developer also reports for its own systems (Anthropic Institute, 2026). As I will keep insisting, however, benchmark skill and real-workflow skill are not the same thing (Chapter 11).
[ai-reviewer: the S9 rework puts the evidential weight the wrong way round. As written, "near-saturation about two years later" reads as an independent public-numbers fact, with the developer's report as secondary echo — but the only /research support for the near-saturation figure is the Anthropic Institute's own self-reported account (R-2, theme 1); the first sweep's SWE-bench entry supports only "far higher figures". Plan task 1.1/S9 asked for the report-supported form with the self-reported caveat attached to the claim itself. The fix must make near-saturation explicitly the developer's self-reported figure, not a fact the developer merely corroborates.]
...
The second half of the "why now" is about money rather than capability.
...
That changes who can take part: a group with modest hardware and no recurring budget can, with care, build workflows of real operational value, a constraint I treat as a design input throughout and return to properly in the closing case studies.
I would add one caution so the economics do not mislead you: cheaper models do not make agentic work cheap, because the cost moves rather than vanishes, shifting into engineering time, into evaluation, and above all into verification, which is where a well-run scientific workflow should expect to spend most of what it saves.
...
Much of the confusion around this technology starts with three words that are used as if they were interchangeable, so let me pin them down and then hold them steady for the rest of the book.
...
That distinction is doing real work, because it underwrites the central recommendation of this book: build agentic workflows, do not deploy autonomous agents.
The reasoning is not timidity; it is the ordinary discipline we already apply to any new instrument.
...
That property, which I call plausible failure, is why verification gets its own part of this book rather than a paragraph, and why Chapter 13 is a gallery of failures rather than a footnote to the successes.
...
Where checking is expensive, slow or subjective (a claim at the research frontier, an interpretive synthesis, an anomaly whose meaning depends on context the system does not hold), fluent output stays dangerous no matter how capable the model, and no amount of benchmark progress moves it across.
...
Multi-step arithmetic done in prose, rather than handed to a tool, fails often enough that delegating it should be a rule, not a preference.
...
Beyond the merely unreliable sits a third category that is not about capability at all, and it is the one I hold fixed while everything else moves: accountability, scientific judgement, and authorship.
An agent cannot be responsible for a flood warning, cannot decide that an anomaly is real rather than an artefact, and cannot be an author of the paper that follows. None of these limits softens as the models improve, because responsibility is not a capability and does not transfer to an instrument, however good the instrument becomes.
The boundary I have drawn here comes with one honest caveat: I am drawing it at the time of writing, and it will move, mostly outward, unevenly, and faster than any publishing schedule. A printed page is the wrong place to track a moving line.
The division of labour in this book is therefore deliberate: the print holds the position and the reasoning, the companion repository tracks the movement, and the verification machinery of Part III is what makes it bearable to work with an instrument whose specification refuses to sit still.
...
I am candid throughout that my examples come from one corner of the environmental sciences; the patterns were chosen, and written, to travel well beyond it.
