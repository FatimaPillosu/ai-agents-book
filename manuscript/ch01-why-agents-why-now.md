...
> **Status:** draft r3 · voice v3.2 (`STYLE.md`) · sentence-per-line per `STYLE.md` §10 · figures as briefs per `FIGURES.md`.
...
One caution is warranted so that the economics are not misread: cheaper models do not make agentic work cheap, because the cost moves rather than vanishes — into engineering time, into evaluation, and above all into verification, which is where a well-run scientific workflow should expect to spend most of what it saves.
