...
*Figure 17.1 — Five requests, in the order that gets them answered, with the disclosure above the count because you read it rather than ask for it. Each request buys a different piece of evidence and costs the authors a different amount, so opening with the most expensive is how a review turns into a standoff. The vermillion panel is the half reviewers get wrong: a transcript is not evidence of what happened, and a re-run asks for a property agentic work does not have. (Rendered as `figures/figure-17-1.svg` from its brief in `fig-brief/ch17-on-the-receiving-end.md`, per `FIGURES.md`.)*
