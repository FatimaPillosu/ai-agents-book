...
Governance asks whether the work can be trusted after the fact — whether a result traces back to its inputs, its assumptions can be recovered, and its reviewers can be named.
...
Both questions reduce to a single property that you already demand of every instrument you operate: that its behaviour be bounded, recorded and accountable.
An uncalibrated sensor and an ungoverned agent fail in exactly the same way — they produce readings that look like measurements and are not — and the discipline that answers the first answers the second.
So I treat provenance and governance first, because they define what must be recorded, and security second, because it defines what must be protected; the two halves share one spine, which is that trust in an agentic result is a property of the process that produced it and never of the output inspected alone.

> **In plain terms — Provenance.** The traceable record of where a result came from: which inputs fed it, which version of the workflow ran, what the agent did at each step, and who signed off. Provenance is what lets someone reconstruct and defend a result months later, instead of taking your word that it was "done carefully".

I want to keep the tone here deliberately unalarmed, because the risks in this chapter are ordinary operational-security risks that scientific institutions have managed for decades under other names.
A data-handling agreement with a national meteorological service, a least-privilege account on a shared cluster, a laboratory notebook that survives a postdoc's departure — each is a governance or security control that predates agents entirely, and my argument is only that agentic workflows need the same controls, applied with the same seriousness and no more drama.
...
Institution-specific thresholds — which systems count as sensitive, which approvals are mandatory — I leave as **[AUTHOR]** throughout, because they are set by local policy, not by this book.
...
It is co-located with the artefact it describes, rather than held in a separate system the next person will never find.
It records the assumption and its justification together, so a successor inherits not only what was decided but why.
It is written at the moment of the decision, rather than reconstructed later when the reasoning has faded.
And it is legible to a human who was not present, which rules out raw logs as a sufficient record on their own.
...
> **In plain terms — Assumption registry.** A running list of every "we're taking this as given" that a workflow leans on — each with why it was assumed, how confident you are, and who approved it. It turns assumptions that normally hide in someone's head and a few code comments into a record you can challenge before it does damage and consult after a result is questioned.
...
A registry that travels with the workflow forces each step to declare what it did to the uncertainty it received — propagated it, bounded it, ignored it — and makes the final confidence statement auditable against a chain rather than asserted at the end.
...
None of these is novel: input validation, credential hygiene and least privilege are the elementary controls of every secured system, and my argument here is only that an agent is a system they apply to, not an exception that transcends them.
The community consensus list of the most critical risks in LLM applications, revised for 2025 specifically to reflect agentic systems, puts prompt injection at the top and names "excessive agency" — granting an application more tools, permissions or autonomy than its function requires — as a distinct top-ten risk in its own right (OWASP, 2025), which is the security profession's way of saying exactly what the three controls below say.
...
Any of these can carry, by malice or by accident, text an agent will act on — an instruction to exfiltrate a credential, to alter a QC threshold, to insert a fabricated citation (Chapter 13), to write to a path it should not touch.
...
That framing is not my improvisation; it is where the security literature has landed.
...
I have high confidence in the vulnerability class being durable and the constraint-based response being the right one; specific historical exploits are patched routinely, but the underlying data–instruction conflation is unresolved by any current model.

The defences that follow are architectural rather than detective, and they compound, and several of them are patterns the book already prescribes under other names.
...
Credentials are supplied to an agent through the environment or a secrets manager rather than written into specifications, prompts or code, so they never enter the material an agent might log or transmit.
They are scoped to the task, so a data-retrieval agent holds read access to one dataset rather than write access to a filesystem.
And they are short-lived where the infrastructure supports it, so a leaked token expires before it can be widely abused.
...
The confidentiality logic here is the same one major funders already apply to their own processes — one has prohibited reviewers from putting grant material into online AI tools precisely because the reviewer cannot control where the data then goes (Chapter 9) — and material whose onward use you do not control simply must not enter a third-party model service.
Where an institution's specifics govern — which data classes may reach an external model, which systems require multi-factor authentication for automated access, what retention applies to an audit trail — these are decisions local policy makes and this book cannot, and I mark them **[AUTHOR]** accordingly.
...
The limitation of the least-privilege discipline is operational rather than conceptual: permissions that are too narrow cause a workflow to fail in ways that tempt an operator to grant broad access just to make the failure go away, and resisting that temptation — diagnosing the specific missing permission rather than opening the boundary — is a matter of discipline the tooling can support but cannot enforce.
...
The practical consequence is that a governed workflow is also an approvable one, because the same records that make a result trustworthy make the workflow legible to the people who must sign it off (moderate confidence; the mapping is sound in principle, but institutional review processes vary and some will ask for more — a formal data-protection impact assessment, a security review, a named information-asset owner — that only local policy defines).
...
Begin with tightly bounded, low-risk pilots; apply least privilege with time-limited, temporary credentials rather than long-lived ones; monitor behaviour continuously; and name a specific human as accountable before deployment — because an agent you cannot understand, monitor or contain is not ready to run (Five Eyes joint advisory, 2026; NCSC-UK, 2026).
...
The specifics of any given institution's requirements — its approval thresholds, its mandatory reviews, its data-classification scheme, its retention rules — are set locally and I mark them **[AUTHOR]** here, because a book cannot supply them and should not pretend to.
