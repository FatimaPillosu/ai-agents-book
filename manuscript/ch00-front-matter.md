...
> **[ai-writer:** both findings are now addressed in the text, and this header is left standing because the escalation underneath them is not mine to close. On the first, "every worked example" is gone. The scope statement now names the three chapters the comment identified, says what in each is the routine half handed to the agent, and says that the experimental judgement staying with the scientist is what keeps them inside the boundary. That is a testable claim rather than a universal one, and it is the weaker and more defensible form the comment pointed at. On the second, "govern" now means one thing in both places: the front matter says the book's kind of governance does not reach exploratory work and names the two weaker disciplines that do, and Chapter 3 §3.7 has been aligned to match, so it no longer asks what "governs" exploration and no longer says its practices "stand in for governance". The **[AUTHOR SIGN-OFF]** marker below is untouched and the framing remains the author's to settle.**]**
...
The worked examples in this book are routine or semi-routine work: quality-control passes on gauge records, verification scores, calibration bookkeeping, manuscript assembly, responses to reviewers.
Three of them reach further, and they are worth naming because they mark the edge of the category rather than sit outside it.
Chapter 8 designs a three-track model intercomparison; Chapter 14 builds a verification toolkit whose figures a partner reports as official; Chapter 15 takes a modelling workflow through to a publication run.
In each, what the agent is given is the routine half: the orchestration, the bookkeeping, the assembly, the explaining.
The experimental judgement stays with the scientist by design, and that is what keeps them inside the boundary.
...
The governance in these pages does not reach that work until a specification becomes possible.
What reaches it instead is weaker: a bounded budget and a contemporaneous record, rather than a check the output has to pass.
Chapter 3 §3.7 sets out both, and says why neither is a check.
