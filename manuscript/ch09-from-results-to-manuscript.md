...
The final stretch of a scientific project eats far more of your time than the science left in it deserves.
Turning verified results into a manuscript, a set of figures, a data statement and a reply to reviewers is mostly not argument but transcription and reconciliation: reading numbers off a results table into the running text, regenerating a figure because an upstream file changed, reformatting citations to a journal's house style, assembling a point-by-point reply to a review that landed four months after you last touched the analysis.
The work is exacting, because a single mismatch between a number in the abstract and the same number in a table is exactly the kind of error a reviewer notices, and it is also the work most exposed to the ordinary failures of tired attention.
My sense is that across the environmental sciences the output side of scholarship absorbs a large fraction of a project's effort while adding almost nothing scientific to it, though I should say plainly that this is seldom measured directly (low-to-moderate confidence) **[AUTHOR: if you have a defensible personal estimate — the share of a recent paper's total effort spent on manuscript assembly, figures and revision rather than analysis — it would anchor this claim; otherwise leave it qualitative]**.

So a tool that can read a pipeline's artefacts, assemble a draft, regenerate a figure from the file that produced its numbers, and draft a structured reply to a review deserves careful rather than reflexive attention — careful precisely because the output side is where the characteristic failure of these systems, fluent and plausible and wrong, does the most damage.
...
The position I take here is therefore narrower than the tooling would allow: agents are drafting and assembly instruments under continuous author control, never authors and never the authority over what the results mean.
I will set out how to generate figures and tables from pipeline artefacts rather than by hand, how to run a reviewer-response workflow in which the human holds sole interpretive authority, how the provenance records of Chapter 12 feed a disclosure statement, and how to keep the whole process on the reliable side of the capability boundary I drew in Chapter 1.

## 9.2 The conventional workflow and where it leaks

The conventional route from results to submitted manuscript is a chain of manual transcriptions between tools that do not talk to one another, and every transcription is a place where the manuscript can quietly drift away from the analysis behind it.
The path runs from analysis scripts producing numbers and arrays, to a plotting step producing figure files, to a word processor into which figures are pasted and numbers are typed, to a reference manager, and finally to a journal's submission system with its own formatting demands.
The artefacts at each stage are copies, not links, and a copy does not update when its source changes.
The failures are familiar to anyone who has assembled a paper under deadline: a figure in the submitted version was generated from an earlier data file than the table beside it; a value quoted in the abstract was updated in the table but not in the text; a significance figure survived from a draft in which the sample was smaller.
None of these is a failure of competence.
They are failures of an architecture that leans on human vigilance to keep half a dozen loosely coupled copies of the same numbers in agreement, and vigilance is exactly the resource a project has least of in its closing weeks (high confidence).

The reviewer-response stage concentrates the same weakness under worse conditions.
A review usually arrives months after the analysis went quiet, so you must first reconstruct the state of a codebase and a dataset you have half-forgotten, then decide for each comment whether it calls for new analysis, a clarification, or a reasoned disagreement, and finally assemble a reply that quotes the reviewer, states your response, and points to the exact changes you made to the manuscript.
Much of the surrounding labour is clerical — collating comments into a table, cross-referencing each to line numbers, re-running an analysis with one parameter changed, regenerating the affected figure — and it lands at precisely the moment when the intellectual core of the task, working out what each comment is really asking and whether it is right, most needs undistracted attention.
...
The redesign swaps manual transcription for regeneration from a single source of truth, and swaps undifferentiated effort for a division of labour that keeps interpretation human.
Its organising principle is simple: every number and every figure in the manuscript should trace to a pipeline artefact and be regenerable from it, so that a late change upstream propagates on its own rather than obliging you to remember every place the old value appeared.
In practice that means figures come from a plotting step that reads the same result files the analysis wrote, tables are generated from those files rather than typed, and the numbers quoted in the running text are, wherever the format allows, drawn from the same artefacts by templating rather than copied by hand.
An assembly agent sits over this arrangement not as an author but as an operator: it can regenerate the figure set when a file changes, assemble a table to the journal's column specification, check that a value quoted in the abstract matches the table it derives from, and flag mismatches for you to resolve.
The interpretive acts — what the result means, which claim it supports, how strongly to state it — stay entirely with you, and the agent's outputs are proposals you accept, edit or reject at an explicit gate (high confidence in the pattern; how much automation is safe varies with the manuscript).

The same principle governs prose drafting, but with a sharper boundary, because prose is where accountability is most easily laundered.
An agent can usefully draft a methods section from a specification and a pipeline configuration, a data-availability statement from provenance records, or documentation of a workflow from its own logs, because these are descriptive tasks whose correctness you can check against artefacts that already exist.
It can also do mechanical language work — enforcing a consistent term, tightening an over-long sentence, aligning citations to a required style — where you read every change.
What it must not do is originate the claims of the paper, decide what the results establish, or produce text whose fluency hides that no human has personally verified the substance behind it.
The discipline that prevents this is that you remain the source of every claim and read every sentence you put your name to, treating agent-drafted prose as a first draft to interrogate rather than a finished product to wave through.
The provenance records of Chapter 12 do double duty here: they are what the methods and data statements are generated from, and they are also the raw material of the disclosure statement that records how agents were used — so a workflow that keeps good provenance for verification gets its disclosure almost for free (moderate confidence).

> **In plain terms — Disclosure statement.** A short note attached to a paper or proposal
> that records how AI tools were used in producing it: which tool did which task, on what,
> and under whose oversight. It exists so an editor or reader can see the human
> accountability behind the work, and it is written from your own records rather than
...
Any treatment of AI use in manuscript preparation has to reckon with the fact that the governing policies are numerous, inconsistent between publishers and funders, and revised faster than a printed book can track, so I survey them by class rather than by name and send you to the repository for current specifics.
...
The first is **disclosure-required**: using AI tools to prepare the manuscript is permitted but must be declared, usually in a designated statement, with varying granularity as to which tools and which tasks you have to name.
The second is **use-restricted**: some tasks are permitted and others are not — a common pattern allows language editing and drafting help while forbidding the generation of figures, data or citations, and some rules restrict use in peer review itself, where a funder may prohibit any generative-AI use in reviewing proposals outright because a confidential manuscript cannot be entered into an external system whose onward data handling the reviewer does not control (NIH, 2023).
The third is **authorship-barred**, which is close to universal and on which I take an unqualified position: an AI system cannot be listed as an author, because authorship entails accountability for the work and an instrument cannot be accountable — the line the major journals drew within weeks of these tools reaching the mainstream, on exactly that accountability rationale (Nature editorial, 2023) (high confidence; this class is stable where the others are volatile).

The practical discipline that follows from a moving landscape is to build the workflow so that it satisfies the strictest plausible reading of all three classes, and to generate the disclosure from records rather than from memory.
Keeping provenance of which agent did which task, on what inputs, under whose review (Chapter 12) means a disclosure statement can be assembled to whatever granularity a venue asks for without reconstructing the history after the fact, and that a restriction you meet late — a journal that forbids agent-generated figures, say — can be checked against the record rather than guessed at.
...
My example concerns a revision of an operational hydrology manuscript after peer review **[AUTHOR: supply the paper and the review — even anonymised — so the specific comments, analyses and responses replace the scaffold below; the workflow shape is general, but the worth of the example is in the real interpretive calls you made]**.
The workflow begins with the agent doing only clerical structuring: it parses the review into a table of individual comments, each with a stable identifier, the reviewer's exact wording quoted verbatim, and a proposed classification into one of a few types — a request for new or changed analysis, a request for clarification, a factual correction, or a point of disagreement.
Classification is a proposal, not a decision.
You read every row and correct the type, because whether a comment is a genuine request for new work or a misunderstanding to be answered in prose is exactly the interpretive judgement the workflow exists to protect.
At this stage nothing about the science has been decided, and the agent has touched no result and originated no claim; it has turned an unstructured document into an auditable list (high confidence in the shape of this step).

The interpretive core is yours alone, and the workflow is arranged so that it happens with full attention rather than in the margins of clerical work.
For each comment you decide the substance of the response: whether the reviewer is right, what if any new analysis is warranted, and how the manuscript should change — and you record those decisions, in your own words, as the authoritative content of the reply.
Where a decision calls for re-running an analysis with one parameter changed and regenerating the affected figure and table, the agent executes that mechanical task against the existing pipeline — the same artefact-linked generation of §9.3 — and you verify the regenerated output against the result files before it is used.
Only once you have settled every response does the agent return to a clerical role, assembling the point-by-point reply: for each comment it lays out the reviewer's quoted wording, your response as written, and a precise pointer to the change made in the manuscript, formatting the whole to the journal's expected structure.
You read the assembled reply in full before submission, because the agent's fluency at producing a well-formed response document is precisely the property that could make an inadequately justified reply read as though it were adequate.
The division of labour is the whole point: the agent does the structuring and assembly at both ends, you do all of the judging in the middle, and the boundary between the two is explicit rather than blurred (moderate-to-high confidence; the exact split depends on the review, and Chapter 13 catalogues what goes wrong when the boundary is allowed to drift).
...
The output side of scholarship has three characteristic failure modes, and each is dangerous precisely because the agent's fluency makes the failed output look finished.
The first is **fabricated citations**: a reference that is plausible in author, journal and year but does not exist, or a real reference bolted onto a claim it does not in fact support.
At the drafting stage the consequence is specific and serious — a fabricated reference can pass straight into a submitted manuscript or a funding proposal — so no citation an agent produces or suggests enters the manuscript in this book's workflow without being resolved to a real, retrieved source and checked against the sentence it supports, the same citation-verification gate set out for literature synthesis in Chapter 5.
Why models invent references, and worked traces of the failure, belong to the failure gallery in Chapter 13; here it is enough that the check is mechanical and external to the model, never the model's own assurance that its reference is real.

The second failure is **style laundering**: agent-smoothed prose that reads as authoritative and, in reading so well, hides whether a human has actually verified the substance behind it — or whose uniform polish sands away the seams where a claim is weak and would, in your own prose, have shown its hesitation.
This one is subtler than fabrication, because nothing in the output is factually wrong on its face; what is lost is the signal that ordinarily tells a reader, and tells you on re-reading, where the argument is thin.
The discipline against it is that you stay the source of every claim and read every sentence critically for whether you can personally stand behind it, treating fluency as no evidence of correctness and refusing to accept a well-turned paragraph just because it is well turned.
The third failure is **over-claiming**: drafted text that states a result more strongly than the evidence carries — a correlation described as an effect, a result on one catchment generalised to a class, an uncertainty range quietly dropped in the abstract.
Over-claiming is the failure most aligned with an agent's disposition to produce confident, readable prose, and the certainty-flag discipline of this book is the direct countermeasure: substantive claims carry an explicit statement of confidence, and a claim whose flag you cannot justify is a claim to weaken or remove.
All three share the property Chapter 1 named at the outset — fluency is uncorrelated with correctness, so a polished output is no evidence of a warranted one — and all three share a remedy: the human, not the agent, is accountable for every claim and checks each against evidence external to the model (high confidence).

## 9.7 Verification checklist

This checklist certifies that a manuscript assembled with agent help is safe to submit; you apply it before submission and again before any revision is returned, and each item is keyed to a failure this chapter has described. It is written to be printable and used without the chapter open.
...
The companion repository holds the perishable and runnable counterparts to this chapter, kept current where the print cannot be.
It carries a minimal artefact-linked manuscript example — a small pipeline whose figures, tables and quoted numbers regenerate from one set of result files — so the pattern of §9.3 can be run rather than only read **[AUTHOR: confirm the runnable example is included and note its path once the `/patterns` layout is settled]**.
...
The division of labour is deliberate, as everywhere in the book: the print states the durable pattern and the reasoning, and the repository carries what changes.
