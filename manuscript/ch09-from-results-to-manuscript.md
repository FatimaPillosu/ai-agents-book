...
The final stretch of a scientific project consumes a share of a scientist's time out of all proportion to the science remaining in it.
Turning verified results into a manuscript, a set of figures, a data statement and a reply to reviewers is mostly not argument but transcription and reconciliation: reading numbers off a results table into the running text, regenerating a figure because an upstream file changed, reformatting citations to a journal's house style, and assembling a point-by-point reply to a review that landed four months after the analysis was last touched.
...
This book contends that across the environmental sciences the output side of scholarship absorbs a large fraction of a project's effort whilst adding almost nothing scientific to it, though this is seldom measured directly (low-to-moderate confidence) **[AUTHOR: if you have a defensible personal estimate — the share of a recent paper's total effort spent on manuscript assembly, figures and revision rather than analysis — it would anchor this claim; otherwise leave it qualitative]**.

A tool that can read a pipeline's artefacts, assemble a draft, regenerate a figure from the file that produced its numbers, and draft a structured reply to a review therefore deserves careful rather than reflexive attention, careful precisely because the output side is where the characteristic failure of these systems, fluent and plausible and wrong, does the most damage.
...
The position this chapter takes is therefore narrower than the tooling would allow: agents are drafting and assembly instruments under continuous author control, never authors and never the authority over what the results mean.
This chapter sets out how figures and tables are generated from pipeline artefacts rather than by hand, how a reviewer-response workflow runs in which the human holds sole interpretive authority, how the provenance records of Chapter 12 feed a disclosure statement, and how the whole process is kept on the reliable side of the capability boundary drawn in Chapter 1.
...
A review usually arrives months after the analysis went quiet, so the author must first reconstruct the state of a half-forgotten codebase and dataset, then decide for each comment whether it calls for new analysis, a clarification, or a reasoned disagreement, and finally assemble a reply that quotes the reviewer, states the response, and points to the exact changes made to the manuscript.
...
Its organising principle is simple: every number and every figure in the manuscript should trace to a pipeline artefact and be regenerable from it, so that a late change upstream propagates on its own rather than obliging the author to remember every place the old value appeared.
...
The discipline that prevents this is that the author remains the source of every claim and reads every sentence carrying their name, treating agent-drafted prose as a first draft to interrogate rather than a finished product to accept unexamined.
The provenance records of Chapter 12 do double duty here: they are what the methods and data statements are generated from, and they are also the raw material of the disclosure statement that records how agents were used, so a workflow that keeps good provenance for verification obtains its disclosure almost for free (moderate confidence).
...
Any treatment of AI use in manuscript preparation has to reckon with the fact that the governing policies are numerous, inconsistent between publishers and funders, and revised faster than a printed book can track, so this chapter surveys them by class rather than by name and directs the reader to the repository for current specifics.
Three broad classes cover most of what a researcher will encounter, and they are worth separating because they impose different obligations.
...
Keeping provenance of which agent performed which task, on what inputs, under whose review (Chapter 12) means a disclosure statement can be assembled to whatever granularity a venue asks for without reconstructing the history after the fact, and that a restriction met late, such as a journal that forbids agent-generated figures, can be checked against the record rather than guessed at.
...
The strongest 2026 capability evidence leaves that rule intact.
...
The author reads every row and corrects the type, because whether a comment is a genuine request for new work or a misunderstanding to be answered in prose is exactly the interpretive judgement the workflow exists to protect.
...
The interpretive core belongs to the author alone, and the workflow is arranged so that it happens with full attention rather than in the margins of clerical work.
For each comment the author decides the substance of the response: whether the reviewer is right, what if any new analysis is warranted, and how the manuscript should change, and records those decisions, in their own words, as the authoritative content of the reply.
Where a decision calls for re-running an analysis with one parameter changed and regenerating the affected figure and table, the agent executes that mechanical task against the existing pipeline (the same artefact-linked generation of §9.3), and the author verifies the regenerated output against the result files before it is used.
Only once the author has settled every response does the agent return to a clerical role, assembling the point-by-point reply: for each comment it lays out the reviewer's quoted wording, the author's response as written, and a precise pointer to the change made in the manuscript, formatting the whole to the journal's expected structure.
...
At the drafting stage the consequence is specific and serious: a fabricated reference can pass straight into a submitted manuscript or a funding proposal.
No citation an agent produces or suggests therefore enters the manuscript in this book's workflow without being resolved to a real, retrieved source and checked against the sentence it supports, the same citation-verification gate set out for literature synthesis in Chapter 5.
...
The second failure is **style laundering**: agent-smoothed prose that reads as authoritative and, in reading so well, hides whether a human has actually verified the substance behind it, or whose uniform polish sands away the seams where a claim is weak and would, in the author's own prose, have shown its hesitation.
This failure is subtler than fabrication, because nothing in the output is factually wrong on its face; what is lost is the signal that ordinarily tells a reader, and tells the author on re-reading, where the argument is thin.
The discipline against it is that the author stays the source of every claim and reads every sentence critically for whether it can be personally stood behind, treating fluency as no evidence of correctness and refusing to accept a well-turned paragraph merely because it is well turned.
The third failure is **over-claiming**: drafted text that states a result more strongly than the evidence carries, such as a correlation described as an effect, a result on one catchment generalised to a class, or an uncertainty range quietly dropped in the abstract.
Over-claiming is the failure most aligned with an agent's disposition to produce confident, readable prose, and the certainty-flag discipline of this book is the direct countermeasure: substantive claims carry an explicit statement of confidence, and a claim whose flag cannot be justified is a claim to weaken or remove.
All three share the property Chapter 1 named at the outset, that fluency is uncorrelated with correctness, so a polished output is no evidence of a warranted one, and all three share a remedy: the human, not the agent, is accountable for every claim and checks each against evidence external to the model (high confidence).

## 9.7 Verification checklist

This checklist certifies that a manuscript assembled with agent help is safe to submit; it is applied before submission and again before any revision is returned, and each item is keyed to a failure this chapter has described. It is written to be printable and used without the chapter open.
