...
The line falls where it does for the reason Chapter 4 §4.4 gives.
The four withheld tasks are the expensive ones to check, and no gain on the generation side makes them cheaper.
That division is a more conservative one than some prominent demonstrations adopted.
...
*Figure 8.3 — What actually changes is who keeps the record. The scientist occupies the same position in both panels because the decisions do not move. What moves is the link between the runs and the log. In the top panel it is broken and rebuilt from memory afterwards; in the bottom panel it is captured as each run launches. (Rendered as `figures/figure-8-3.svg` from the brief below, per `FIGURES.md`.)*
