...
I want to do something in this chapter that the previous fourteen could not: show the book's apparatus working as one piece, rather than catalogued part by part.
Each earlier chapter isolated a single instrument — the specification schema of Chapter 3, the roster derivation of Chapter 10, the registries and gates of Chapter 12, the evidential hierarchy of Chapter 11, the manuscript pipeline of Chapter 9 — and examined it on its own so that its logic was legible.
That isolation is a teaching convenience and, past a point, an operational fiction, because the instruments only earn their cost when they interlock: the audited output of each stage is the admissible input to the next, and a gap between two stages is exactly where governance fails without anyone noticing.
The workflow I follow here is real and was executed end to end **[AUTHOR: name the modelling problem — the physical quantity modelled, the spatial domain and period, the scientific question the study set out to answer, and the publication it produced]**, and I have deliberately structured the account as a walk through its stages.
At each stage I state four things in the same order: what was specified, which agent role acted, which gate applied, and — the load-bearing question throughout — what I, the human, decided.
The single claim this chapter exists to demonstrate is that human authority in a governed agentic workflow is not concentrated at the end, in a final sign-off, but distributed across every gate, and that this distribution is what makes the workflow a scientific instrument rather than an automation (high confidence, on the composition argument developed below).
...
I have drawn them left to right for legibility, but the lifecycle is not a pipeline that runs once cleanly, because every gate has a failure exit that returns work to the role that produced it, and a registry entry made late can invalidate a stage already passed.
...
The workflow began, as every governed workflow in this book begins, with a written specification rather than a conversation, and that specification was the artefact everything downstream was held to.
Following Chapter 3, I first decomposed the modelling task into units small enough that each had a single checkable outcome, then wrote each unit to the seven-field schema — objective, inputs, acceptance criteria, stop conditions, assumptions and conventions, provenance requirement, and named reviewer **[AUTHOR: list the units the modelling task decomposed into — for example data acquisition and QC, calibration, the model runs themselves, evaluation, and figure generation — and state how many units the full specification contained]**.
...
I placed the specification under version control and had it reviewed before any agent executed against it, which is the practice Chapter 3 argues moves your scepticism to where it earns the highest return: I treated a change to the specification as a change to the method, requiring the same care and leaving the same record, so that the question an auditor would later ask — what was this workflow trying to do, and who decided — is answered by an artefact and not by memory (high confidence in the practice; its adequacy on this workflow is mine to confirm).

## 15.3 Deriving the roster, not choosing it (Chapter 10)

I derived the agent roster from the specification rather than assembling it by preference, which is the discipline Chapter 10 sets against the temptation to add agents because more of them feels more capable.
...
The audit trail beneath both registries, in Figure 15.1, is not a third artefact but the union of the specification, the gate outcomes and the two registries, timestamped and attributable, and it is the substrate on which the publication run of §15.7 builds its disclosure.
...
A single stage examined at full resolution shows how the pieces of the preceding three sections act together in time, and Figure 15.2 traces one representative gated stage from specification unit to my decision at the gate.
The stage I have chosen for this walk is **[AUTHOR: name the representative stage — for example the calibration stage, or the observational quality-control stage — and state its objective in one line]**, and it is representative precisely because nothing about it is exceptional: it is one unit among several, governed by the same schema and the same gate discipline as the rest.
...
A pass advances the output to the next unit; a failure returns it to the producing agent with the reason recorded, and the loop repeats within the budget the stop conditions set, halting and handing back to me if that budget is exhausted.
My decision sits at the gate, not after it: I am the party who accepts a passed stage into the workflow's record or overrides a gate outcome with a reasoned entry of my own **[AUTHOR: describe one real decision you took at this gate — an acceptance, an override, or a return — and what the output and provenance were that you decided on]**.
The point the stage makes concrete is that I am not reviewing a finished workflow at the end but authorising it one gate at a time, and that the cost of this — a decision per gate rather than a decision per project — is the price of an instrument whose every step is attributable (high confidence in the pattern; the per-stage effort on this workflow is mine to report).
...
I mitigated it in two ways that Chapter 12 recommends — by giving the reviewer the specification and the acceptance criteria but not the producing agent's reasoning trace, so the review tested the output against the standard rather than against the producer's justification for it, and by recording reviewer coverage as a first-class artefact so an unreviewed unit was visible as a gap rather than hidden as an omission.
This is the same discipline the field already applies to its models: when meteorologists assessed a vendor-published weather model, they did not take its self-reported scores on trust but re-verified it on their own analyses, under operational-like conditions, with their own metrics before trusting it (Ben Bouallègue et al., 2024) — independent re-verification on your own ground, which is exactly what a reviewer role does for a unit of agent work.
The claim to make carefully is that independent review raises the evidential tier of a workflow's outputs in the sense of Chapter 11 — moving a result from asserted-correct to checked-by-an-independent-party — without ever reaching the top of that hierarchy, because agent review is a lower tier of evidence than expert human scrutiny and does not displace it (moderate confidence; the strength of the independence obtained on this workflow is mine to characterise, and the reviewer-coverage record is the evidence that would support or qualify the claim).
...
That last point is not my preference but settled policy: journals established early that an AI tool cannot be listed as an author, because authorship carries an accountability a tool cannot bear, and that researchers must instead document how such tools were used (Nature editorial, 2023) — the same category distinction this book has held since Chapter 1, that authorship and accountability are errors of category to assign to an instrument.
...
Journal and funder policy on AI use is a volatile landscape, and I treated it as Chapter 9 prescribes — surveyed by policy class in the manuscript, with the specific current requirements kept in the companion repository — because by 2026 the requirement to disclose substantive AI assistance had become common but not uniform across venues **[verify: characterise the disclosure-policy landscape at submission, and the specific venue's requirement, in the repository]**.
The implication worth stating is that a governed workflow makes disclosure cheap and honest, because the material a disclosure statement needs already exists as the audit trail, whereas an ungoverned workflow must reconstruct under deadline a history it did not keep — and reconstruction under deadline is where over-claiming and style laundering enter (high confidence in the mechanism; the reception of this particular disclosure is mine to report).

## 15.8 Where authority sat, and what it cost

The single lesson of the walk is that human authority was distributed across the workflow rather than concentrated at its end, and that the distribution is the governance rather than a supplement to it.
At the specification stage I decided what the work was and what would count as done; at the roster stage I decided which actors would act and within what bounds; at each gate I accepted, overrode or returned a passed output; at the review stage I commissioned and read an independent check; and at publication I stood as the sole author and the sole party accountable for the interpretation.
No single one of these decisions is novel — each is the ordinary exercise of a scientist's judgement — and that is the point: the apparatus of this book does not introduce a new kind of authority but relocates the familiar kind to the points in an agentic workflow where it can still be exercised meaningfully, which are the gates rather than the steps between them.
...
*Chapter 16 turns from one governed workflow to the harder question of how a whole research group starts: a capability-based on-ramp, an honest cost model, the roles a group needs, and the institutional, ethical and energy considerations that adoption brings.*
