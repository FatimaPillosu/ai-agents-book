...
This chapter attempts something the previous fourteen could not: to show the book's apparatus working as one piece, rather than catalogued part by part.
...
At each stage the account states four things in the same order: what was specified, which agent role acted, which gate applied, and, the load-bearing question throughout, what the human decided.
...
The governed lifecycle followed here has five stages, and Figure 15.1 sets them out as one architecture before the sections examine each in turn.
...
The agent roster was derived from the specification rather than assembled by preference, which is the discipline Chapter 10 sets against the temptation to add agents because more of them feels more capable.
...
A single stage examined at full resolution shows how the pieces of the preceding three sections act together in time, and Figure 15.2 traces one representative gated stage from specification unit to the human decision at the gate.
...
At the specification stage the human decided what the work was and what would count as done; at the roster stage the human decided which actors would act and within what bounds; at each gate the human accepted, overrode or returned a passed output; at the review stage the human commissioned and read an independent check; and at publication the human stood as the sole author of, and the sole party accountable for, the interpretation.
No single one of these decisions is novel, each being the ordinary exercise of a scientist's judgement, and that is the point: the apparatus of this book does not introduce a new kind of authority but relocates the familiar kind to the points in an agentic workflow where it can still be exercised meaningfully, which are the gates rather than the steps between them.
