...
*Figure 13.6 — The claim reads the same on both sides of the evidence's edge. That is what makes extrapolation the sharpest form of plausible failure: confidence does not drop where the support runs out. So the gate never judges how convincing the claim sounds; it compares the claim's scope against the evidence's scope, and anything reaching past gets demoted to hypothesis and sent for independent test, not deleted. More assured phrasing never moves it back up. New evidence does. (Rendered as `figures/figure-13-6.svg` from the brief below, per `FIGURES.md`.)*
