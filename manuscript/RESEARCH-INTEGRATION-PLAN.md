...
- **G7 — Word-growth caps.** Each chapter has a cap in §5 (net growth over its current length). The caps sum to roughly 2,300 words book-wide, excluding `FURTHER-READING.md`. A cap is a ceiling, not a target: if an instruction can be met in fewer words, use fewer.
  [ai-reviewer (post-execution): this rule needs an explicit basis a future agent cannot misread. The caps are only satisfiable read as *prose-only* growth — several tasks mandate three to six new references at 25–50 words each, which would consume most of a small cap before any mandated sentence was written — and the reviewer has adjudicated them prose-only accordingly. Even so, measured prose-only growth exceeded the cap in ch02 (~+109/90), ch10 (+198/170), ch11 (+526/430), ch12 (~+299/260), ch13 (+177/150) and ch16 (~+182/170), in each case traceable to mandated task content rather than padding; accepted, but the editor should state the counting basis and set caps that fit the mandated content next time. Also: the stated book-wide sum of the §5 caps is 2,580, not "roughly 2,300".]
...
[ai-reviewer (post-execution): two gaps in this section a future agent will trip on. (1) It fixes the references format but not the *in-prose* citation marker, and three variants emerged across the seven video-citing chapters — "(practitioner commentary; see the references)", "(practitioner commentary)", "(practitioner commentary, 2026)"; ai-editor should decree one form. (2) "Creator (2026)" leaves the surname convention open — chapters render the same person as both "Jones, N. B." and "Nate B Jones"; one convention should be fixed for chapters and FURTHER-READING alike.]
