...
> **[ai-reviewer: A1 review — 7 comments in this file,** the heaviest count in the pass, for the heaviest batch. All eight tasks landed and the six-tier renumbering is complete: every site in the plan's D3 table is updated, and `five-tier`, `Five tiers` and `independent adversarial scrutiny` return nothing anywhere in the manuscript. `REVISION-PLAN.md` line 103 is correctly untouched. No new citation. The new AUTHOR marker at §11.5 is present. Word budgets ran 14–20% over on tasks 11.2, 11.3 and 11.4, which is inside G7's stated tolerance and is not a finding. **The most serious finding is the first below: both of the in-book instances the new Tier 5 nominates appear to be disqualified by the tier's own honest-limit clause.** Also here: a five-sentence caption against `FIGURES.md`'s two or three, a checklist item the renumbering missed, an unsourced universal claim about instrument certificates, and an unbaselined superlative in §11.7.**]**
...
[ai-reviewer: the most serious substantive finding in the pass. The paragraph above sets a test that the two instances below appear to fail, and the section asserts the independence it has just told the reader to argue for.
Take §11.3 first. Operational analyses are produced by assimilating station observations. Scoring a forecast model against both is standard and valuable practice, and the two error structures genuinely differ, but they are not unrelated: the analysis inherits the observations. That is structurally the case the honest-limit sentence names, "two retrievals calibrated against the same ground network", and it is named four lines above as an example of what does not corroborate. The claim may well survive — the argument would be about what the assimilation does and does not propagate — but the book has to make it rather than assert it, because it has just told the reader that the argument belongs in the record.
Chapter 1 §1.2 is the harder case. The two "independent measurement methods" are a frontier-model developer's benchmark saturation and an evaluation organisation's task-length doubling. §1.2's own preceding sentence reads "Both parties have an interest in demonstrating progress, so neither number is disinterested", and the `/research` entry records METR's task suite as "predominantly software-engineering", which is the same domain the other measure is drawn from. A shared incentive and an overlapping task domain are two hidden dependencies, not zero. Nominating this as an instance of the book's second-strongest evidential tier hands a sceptical reader the tier's own refutation in the tier's own words.
This matters more than a wording problem because F5's whole case for adding the tier was that the book already uses independent-method corroboration as its strongest move. If neither nominated instance is clean, the tier is still right and the evidence for its familiarity is not. Three ways out, all ai-writer's: argue the independence in each case, replace one or both instances with a cleaner one from the manuscript, or say plainly that these are partial corroborations and use them to demonstrate the honest limit rather than the tier. The third would be the most in the book's own voice. Cross-referenced at Ch. 14 §14.3, which makes the same claim about the same case at greater length.]
...
[ai-reviewer: the re-brief is otherwise thorough — six bars in every field including the alt-text and the infographic description, both new annotations present, the bracket moved to 3–6, the footer arithmetic corrected to tier-5, and the reviewer purple and icon kept on the top bar as instructed. Two things to fix. First, the caption now runs to the title line plus five sentences against `FIGURES.md` §6.1's "two or three"; the pre-A1 caption was title plus three. Two of the five are new and both are worth keeping in substance, so this is a compression job rather than a deletion job, and the tier-six sentence is the one carrying the least that the annotation does not already carry on the canvas. I have flagged the underlying ambiguity in `FIGURES.md` — the guide does not say whether the title line counts — but on either reading five is over. Second, a legibility risk the plan anticipated and the brief did not resolve: §6.2 of the plan warned that six bars plus a standfirst plus six annotations sits at the density ceiling of `FIGURES.md` §2, and instructed that if the canvas crowds, the tier-1 and tier-2 annotations go rather than the two new ones. The brief carries eight annotations plus a two-line footer and says nothing about that fallback, so whoever renders it has no instruction. One line in the brief would carry the decision forward.]
...
[ai-reviewer: a paragraph-rhythm note on the block that starts here and runs to the info-box. Its four prose paragraphs measure 67, 65, 69 and 83 words, so three of the four sit below `STYLE.md` §2's 80–200 band and they sit below it consecutively. §2 says "roughly", and one short paragraph doing rhetorical work is explicitly allowed; three in a row is a different thing, and it makes the passage read as a sequence of notes rather than as the developed argument the surrounding section is written in. The material is right and the order is right — the hole, the rule, the definition, the detector, the honest limit — so this is about where the breaks fall, not about what is said. Compare the §11.5 paragraphs above it, which run 100 to 150 words. Ai-writer's to rebalance.]
...
[ai-reviewer: a universal factual claim about metrology practice, carrying no source, no `[verify]` and no confidence flag, in a chapter whose integrity note says report-sourced citations carry a DOI or URL. The `/research` reports contain nothing on calibration certification, which the plan's G1 anticipated for other subjects and did not anticipate for this one, so the task instruction reproduced the review's phrasing without asking where the fact comes from. The substance is very likely right, which is exactly why it should be marked rather than waved through: an environmental readership includes people who hold calibration certificates and will know whether "every" is true of theirs. Two honest forms are available and neither costs anything — soften the quantifier and flag the confidence, or keep the quantifier and carry `[verify]` against it. I am not proposing wording; the choice between the two is a judgement about how much the argument needs the universal.]
...
*Figure 11.3 — Calibrating the gate like the instrument it is. You plant faults you know about, run the gate blind, and count what it misses, class by class. The two honesty rules are on the canvas. A clean sweep on twenty seeded faults still leaves a possible miss rate near fifteen percent. A gate that never fires on real work is evidence of a broken gate, not of flawless upstream work. (Rendered as `figures/figure-11-3.svg` from the brief below, per `FIGURES.md`.)*
...
[ai-reviewer: "the best value on the ladder" is a superlative comparing six tiers whose costs the section has just described qualitatively and never priced, so the ranking rests on nothing the reader can check. That is the unbaselined-comparative pattern of `STYLE.md` §12.1(c), and it appears in the one paragraph of the chapter whose whole subject is cost. It also carries no confidence flag, in a section where the four surrounding tier-cost statements carry their warrant in the sentence ("warranted wherever…", "reserved for claims whose failure would be consequential"). The plan's own wording for task 11.6(a) is where the phrase came from, which does not make it compliant. The claim is plausible and probably worth keeping in substance; it needs either a baseline or a flag. Ai-writer's.]
...
[ai-reviewer: the renumbering missed this item. Its list of authority sources runs schema (Tier 1), invariant (Tier 2), held-out data (Tier 3) and independent reviewer (now Tier 6), and it silently omits both the transfer regime and the new corroborating method. §11.3's own list one page above was correctly extended with "the corroborating method at Tier 5 is chosen for an error structure unlike the first one's", so the checklist is now a degraded echo of a passage that was fixed. G9 asks that renumbering be mechanical and total, and this is the one site where it was not. It matters more than a normal omission because the checklist is written to be applied by a colleague from the record alone, so a Tier 5 claim checked against this item has nothing to check against. Ai-writer's to extend.]
