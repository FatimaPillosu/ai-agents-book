...
Clean independence is harder to come by than the definition makes it sound, and this book's own two attempts show why.
§11.3 scores a data-driven weather model against operational analyses and against independent station observations.
The two references do have different error structures, which is why an operational centre uses both.
They are not unrelated: an operational analysis is produced by assimilating station observations, so part of the second reference sits inside the first.
Chapter 1 §1.2 sets a model developer's report of benchmark saturation beside an evaluation organisation's task-length doubling.
The two measurement approaches genuinely differ, and both parties have an interest in the same answer.
Both task suites are also heavily software-engineering work, so the domains overlap as well.
A shared incentive and an overlapping domain are two dependencies, not none.

Neither pairing is worthless, and neither reaches the tier by simply existing.
Each is a partial corroboration, and saying so is what the tier asks of you.
Name the shared dependency, state what it does and does not carry across, and weight the agreement by what is left.
The argument is what carries the evidence here, more than the second determination does.
Two references that are merely not identical do not establish Tier 5 (high confidence).
Chapter 14 §14.3 works the weather-model pairing through in exactly that form, as the case study's strongest evidential move and an honest account of what weakens it.
...
A calibration certificate on a physical instrument normally answers this with something these tiers so far lack: a validity period.
The conventions vary by instrument class and by accrediting body, and no source behind this book covers them.
Read it as the book's own observation rather than a cited fact (moderate-to-high confidence).
Record a gate calibration the same way: with the date it was made and a validity window the group has set.
...
What the detector cannot say is what moved.
...
In the environmental sciences that is often an existing dataset rather than new work.
The cost is then the analyst time to reconcile two sources, not the cost of collecting either.
Where that holds, Tier 5 is warranted for any quantitative result headed for publication.
It buys the strongest evidence on the ladder short of expert attention, at a cost closer to Tier 3's (moderate confidence; reconciliation cost varies a great deal by variable and by network).
...
- **Checks external to the producer.** No tier was established by the agent that produced the output, an identical agent, or the same model checking itself; each check draws authority from a schema (Tier 1), an invariant (Tier 2), held-out data (Tier 3), a transfer regime the evaluator chose (Tier 4), a corroborating method with an unlike error structure (Tier 5), or an independent reviewer (Tier 6) (§11.3; Chapters 7, 10). For a Tier 5 claim the record shows the argument for that unlikeness, not just the name of the second dataset.
