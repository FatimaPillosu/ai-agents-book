...
The problem I want to take as this chapter's case study is an ordinary one made difficult by a single constraint that reshapes everything downstream: the observations against which a rainfall forecast must be judged cannot leave the institution that holds them.
...
> **In plain terms — Data sovereignty.** The principle that a dataset stays under the legal and
...
The design response I developed inverts the usual direction of movement: rather than bringing the observations to the verification tools, it brings the verification tools to the observations, packaged so that they run entirely inside the partner's own environment and emit only what the partner chooses to share — typically aggregate scores, never the underlying records.
...
> **In plain terms — Open-weight model.** A language model whose trained parameters — its
> "weights" — are published, so you can download it and run it on your own hardware, inside your
...
The limitation of this tier is that it reintroduces a human bottleneck and a dependence on the team's availability, which is precisely the dependence the deterministic and tutoring tiers were designed to minimise; I accept this deliberately, on the reasoning that eliminating the bottleneck entirely would mean either sending the observations to the specialists — forbidden — or letting a local model make specialist judgements it cannot be trusted to make, which is the failure mode the whole architecture exists to prevent.
...
The convergence of a governance-first design and a constraint-first design is the finding this chapter most wants to leave with you, because it suggests that the discipline the well-resourced can *choose* to adopt, the under-resourced are *forced* into — and the forced version is not obviously worse.

The limitation of this case study, stated plainly, is that it is a single deployment in a specific setting, and you should treat its architecture as a transferable pattern rather than its results as a general proof.
